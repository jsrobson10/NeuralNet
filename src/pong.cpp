
#include "pong.hpp"
#include "random.hpp"

#include <iostream>

Pong::Pong(Brain& brain)
{
	this->paddle_l_p = 0;
	this->paddle_r_p = 0;
	this->paddle_l_v = 0;
	this->paddle_r_v = 0;
	this->ball_p = Vector(0, 0);
	this->ball_v = Vector(1, 0.5);
	this->brain = &brain;
	this->state = 0;
	this->left = 0;
	this->at = 0;

	//rewards pos
	for(int i = 0; i < s_rewards.size(); i++)
	{
		int x = i / 2;
		int y = i % 2;

		s_rewards[i].pos = Vector(x * 64 - 96, y * 64 + 96);
		brain.reg_sensor(&s_rewards[i]);
	}

	m_paddle_l[0].pos = Vector(-32 - 64, -192);
	m_paddle_l[1].pos = Vector(-32 - 0, -192);
	m_paddle_r[1].pos = Vector(32 + 0, -192);
	m_paddle_r[0].pos = Vector(32 + 64, -192);

	brain.reg_sensor(&s_ball);
	brain.reg_sensor(&s_paddle_l);
	brain.reg_sensor(&s_paddle_r);
	brain.reg_motor(&m_paddle_l[0]);
	brain.reg_motor(&m_paddle_l[1]);
	brain.reg_motor(&m_paddle_r[0]);
	brain.reg_motor(&m_paddle_r[1]);
}

double Pong::shock_nve()
{
	return (Random::num() < 1.0/32.0) ? 4 : 0;
}

double Pong::shock_pve()
{
	double v = ((at % 32) == 0) ? 4 : 0;

	return v;
}

void Pong::update_game()
{
	ball_p += ball_v;

	if(ball_p.y > 48 || ball_p.y < -48)
	{
		ball_v.y *= -1;
	}

	if(ball_p.x > 160 && std::abs(ball_p.y - paddle_r_p) < 16)
	{
		ball_v.x = -1;
		state = 2;
		left = 128;
	}
	
	if(ball_p.x < -160 && std::abs(ball_p.y - paddle_l_p) < 16)
	{
		ball_v.x = 1;
		state = 2;
		left = 128;
	}

	else if(ball_p.x > 192 || ball_p.x < -192)
	{
		left = 256;
		ball_p = Vector(0, 0);
		ball_v = Vector(Random::num() < 0.5 ? -1 : 1, Random::num() < 0.5 ? -0.5 : 0.5);
		paddle_l_v = 0;
		paddle_r_v = 0;
		paddle_l_p = 0;
		paddle_r_p = 0;
		state = 1;
		return;
	}

	if(m_paddle_r[0].get() > m_paddle_r[1].get())
	{
		paddle_r_v -= 1;
	}

	else
	{
		paddle_r_v += 1;
	}

	if(m_paddle_l[0].get() > m_paddle_l[1].get())
	{
		paddle_l_v -= 1;
	}

	else
	{
		paddle_l_v += 1;
	}

	if(m_paddle_l[1].get() > 1 && m_paddle_l[1].get() > m_paddle_l[0].get())
	{
		paddle_l_v += 1;
	}

	if(paddle_r_p < -48 && paddle_r_v < 0)
	{
		paddle_r_v = 0;
	}

	if(paddle_l_p < -48 && paddle_l_v < 0)
	{
		paddle_l_v = 0;
	}

	if(paddle_r_p > 48 && paddle_r_v > 0)
	{
		paddle_r_v = 0;
	}

	if(paddle_l_p > 48 && paddle_l_v > 0)
	{
		paddle_l_v = 0;
	}

	paddle_l_p += paddle_l_v;
	paddle_r_p += paddle_r_v;
	paddle_l_v *= 15.0/16.0;
	paddle_r_v *= 15.0/16.0;
	s_paddle_l.pos = Vector(-192, paddle_l_p - 48);
	s_paddle_r.pos = Vector(192, paddle_r_p - 48);
	s_ball.pos = ball_p - Vector(0, 48);
	s_paddle_l.set(shock_pve());
	s_paddle_r.set(shock_pve());
	s_ball.set(shock_pve());

}

void Pong::update_nve()
{
	s_ball.set(shock_nve());
	s_paddle_l.set(shock_nve());
	s_paddle_r.set(shock_nve());

	for(Sensor& s : s_rewards)
	{
		s.set(shock_nve());
	}

	left -= 1;

	if(left == 0)
	{
		state = 0;
	}
}

void Pong::update_pve()
{
	for(Sensor& s : s_rewards)
	{
		s.set(shock_pve());
	}

	left -= 1;

	if(left == 0)
	{
		state = 0;
	}
}

void Pong::update()
{
	switch(state)
	{
		case 0:
			update_game();
			break;
		case 1:
			update_nve();
			break;
		case 2:
			update_game();
			update_pve();
			break;
	}

	at += 1;
}

